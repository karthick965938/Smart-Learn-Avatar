...
void app_main(void)
{
    ...
    if (err != ESP_OK) {
        ...
        if (err != ESP_OK || buf_len_long == 0) {
            ESP_ERROR_CHECK(nvs_set_str(my_handle, "KB_url", CONFIG_KB_URL));
            ESP_ERROR_CHECK(nvs_commit(my_handle));
            ESP_LOGI(TAG, "no KB url, give a init value to key");
        } else {
            ESP_LOGI(TAG, "stored KB url:%s", kb_url);
        }
        ...
        if (err != ESP_OK) {
            ...
            ESP_ERROR_CHECK(nvs_commit(my_handle));
            ESP_LOGI(TAG, "no bg_color, give a init value: 0x%06X", bg_color);
        } else {
            ESP_LOGI(TAG, "stored bg_color:0x%06X", bg_color);
        }
        ...
        if (err != ESP_OK) {
            ...
            ESP_ERROR_CHECK(nvs_commit(my_handle));
            ESP_LOGI(TAG, "no text_color, give a init value: 0x%06X", text_color);
        } else {
            ESP_LOGI(TAG, "stored text_color:0x%06X", text_color);
        }
        ...
        if (err != ESP_OK) {
            ...
            ESP_ERROR_CHECK(nvs_commit(my_handle));
            ESP_LOGI(TAG, "no button_color, give a init value: 0x%06X", button_color);
        } else {
            ESP_LOGI(TAG, "stored button_color:0x%06X", button_color);
        }
    }
    nvs_close(my_handle);
    ...
    while (1) {
        ...
        if (bits & NVS_MODIFIED_BIT) {
            ...
            err = nvs_get_str(my_handle, "KB_url", kb_url, &buf_len_long);
            if (err != ESP_OK) {
                ESP_LOGE(TAG, "Failed to read 'KB_url' from NVS: %s", esp_err_to_name(err));
                nvs_close(my_handle);
                return;
            }
